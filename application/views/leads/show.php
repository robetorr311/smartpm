...
<div id="show-section" class="container-fluid show-edit-visible">
    <div class="row page-header-buttons">
        ...
        <div class="col-md-4 text-right">
            <a <?= $prev_lead ? 'href="' . base_url('lead/' . $prev_lead->id) . '"' : '' ?> class="<?= $prev_lead ? 'btn btn-info btn-fill' : 'btn btn-default btn-fill' ?>"><i class="fa fa-angle-double-left" aria-hidden="true"></i>&nbsp; Prev Lead</a>
            <a <?= $next_lead ? 'href="' . base_url('lead/' . $next_lead->id) . '"' : '' ?> class="<?= $next_lead ? 'btn btn-info btn-fill' : 'btn btn-default btn-fill' ?>">Next Lead &nbsp;<i class="fa fa-angle-double-right" aria-hidden="true"></i></a>
        </div>
    </div>
    <div class="row">
        <div class="col-md-8">
            ...
            <div class="card">
                <div class="header">
                    ...
                    <div class="clearfix" style="padding: 10px;"></div>
                    <?php if ($lead->sales_rep_name !== null) { ?>
                        <h4 class="title" style="float: left;">Sales Rep</h4>
                        <span class="status">
                            <?= empty($lead->sales_rep_name) ? '-' : $lead->sales_rep_name ?>
                        </span>
                        <div class="clearfix" style="padding: 10px;"></div>
                    <?php } ?>
                </div>
                        ...
                        <select class="form-control" id="lead" name="status">
                            ...
                            <optgroup label="Leads">
                                <?php foreach ($status_lead as $status_index) { ?>
                                    <option value="<?php echo $status_index ?>" <?= $lead->status == $status_index ? ' selected' : '' ?>><?= LeadModel::statusToStr($status_index) ?></option>
                                <?php } ?>
                            </optgroup>
                ...
                <div class="header">
                    ...
                    <div class="content">
                        <select class="form-control lead-status" id="sales_rep_id" name="sales_rep_id" style="width: 100%;">
                            <option value="" disabled <?= empty($lead->sales_rep_id) ? 'selected' : '' ?>>Select Sales Rep</option>
                            <?php foreach ($users as $user) : ?>
                                <option value="<?= $user->id ?>" <?= ((!empty($lead->sales_rep_id)) && $user->id == $lead->sales_rep_id) ? 'selected' : '' ?>><?= $user->name . ' (@' . $user->username . ')' ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    <div class="clearfix"></div>
