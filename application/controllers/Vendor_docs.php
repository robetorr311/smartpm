...
class Vendor_docs extends CI_Controller
{
	public function __construct()
	{
		...
		$this->load->model(['VendorsDocModel', 'Common_model', 'VendorModel']);
		$this->doc = new VendorsDocModel();
	...
	public function index($vendor_id)
	{
		...
		$vendor = $this->vendor->getVendorById($vendor_id);
		$params = array();
		$params['vendor_id'] = $vendor_id;
		$params['is_active'] = 1;
		$docs = $this->Common_model->get_all_where('vendors_doc', $params);
		$this->load->view('header', ['title' => 'Vendor Doucment']);
		$this->load->view('vendors_doc/index', [
			'vendor' => $vendor,
			'docs' => $docs
		]);
	...
	public function ajaxupload_vendordoc()
	{
		authAccess();
			...
			foreach ($_FILES['doc']['name'] as $key => $filename) {
				...
				if (in_array($file_ext, $allowed_extension)) {
					if ($file_ext != 'zip') {
						...
						$targetPath = "assets/vendor_doc/" . $new_name;
	...
	public function ajaxsave_vendordoc()
	{
		authAccess();
	...
	public function deletedoc()
	{
		...
		$posts = $this->input->post();
		$this->db->query("UPDATE vendors_doc SET is_active=0 WHERE id='" . $posts['id'] . "'");
		return true;
	}
